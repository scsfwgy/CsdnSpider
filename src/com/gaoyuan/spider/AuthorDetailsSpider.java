...
public class AuthorDetailsSpider extends BaseSpider {
	...
	public void process(Page page) {
		...
		if (page.getUrl().regex(Contents.Blog.blogAuthor_regex).match()) {
			/*
			 * header
			 */
			...
			// System.out.println(TAG + originalNums + repuishNums +
			// translateNums
			// + commentNums);
			...
			author.setBlogExpert(isBlogExpert);
			author.setBlogStars(isBlogStars);
			author.setColumnUp(isColumnUp);
			author.setCommentNums(commentNums);
			author.setDescb(describe);
			author.setHeadImg(headImg);
			author.setId_author(id_author);
			author.setMicrMvp(isMicrMvp);
			author.setName(name);
			author.setOriginalNums(originalNums);
			author.setPersist(isPersist);
			author.setPoints(points);
			author.setPreBlogExpert(isPreBlogExpert);
			author.setRank(rank);
			author.setRepuishNums(repuishNums);
			author.setTitle(title);
			author.setTranslateNums(translateNums);
			author.setViewNums(viewNums);
			author.setId(MyStringUtils.getGUID());
			if(isLog())
				System.out.println(author.toString());
			//save 
			boolean isSave = false;
			if(isDb())
			 isSave=iAuthorDetailsService.saveAuthorDetails(author);
			if(isDbLog())
			System.out.println("保存作者详情："+isSave);
		}
	}
	...
	public static void main(String[] args) {
		AuthorDetailsSpider authorDetailsSpider= new AuthorDetailsSpider();
		authorDetailsSpider.process(authorDetailsSpider,"wgyscsf");
	}

	public void process(AuthorDetailsSpider authorDetailsSpider ,String userId) {
		Spider.create(authorDetailsSpider).addPipeline(null)
				.addUrl(Contents.Blog.blogAuthor_rootUrl + userId)
				.thread(1)
				.run();
	}
